"""App Store: attach build 69 to a 5.0.19 version, set What's New in every locale, submit for review.
...
dry = "--dry" in sys.argv
notes = json.load(open("/Users/jordyverwerft/dev/MigraineMe/tools/release/release_notes_5019.json"))
ALIAS = {"en-GB": "en-US", "es-MX": "es-ES", "pt-BR": "pt-PT", "it": "it-IT"}
def note_for(loc): return notes.get(loc) or notes[ALIAS[loc]]
# 1. build 69 processed?
b = None
for _ in range(40):
    r = request("GET", f"/v1/builds?filter[app]={APP}&filter[version]=69&fields[builds]=version,processingState,usesNonExemptEncryption")
    if r.get("data"):
        b = r["data"][0]; print("build 69:", b["attributes"]["processingState"])
        if b["attributes"]["processingState"] == "VALID": break
    else: print("build 69 not visible yet")
    if dry: break
    time.sleep(60)
if dry: sys.exit(0)
...
# 2. version 5.0.19
v = request("GET", f"/v1/apps/{APP}/appStoreVersions?filter[versionString]=5.0.19&fields[appStoreVersions]=versionString,appStoreState")
if v.get("data"):
    vid = v["data"][0]["id"]; print("version exists", vid, v["data"][0]["attributes"]["appStoreState"])
else:
    c = request("POST", "/v1/appStoreVersions", {"data": {"type": "appStoreVersions", "attributes": {"platform": "IOS", "versionString": "5.0.19", "releaseType": "AFTER_APPROVAL"},
