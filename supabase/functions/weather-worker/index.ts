 ...
 * Only writes columns for weather sub-metrics that are enabled in metric_settings.
 */
async function upsertUserWeatherDay(
  ...
  timezone: string,
  nowIso: string,
  enabledWeatherMetrics: Set<string>,
  pollenRow?: any,
  airRow?: any
): Promise<void> {
  const tempOn    = enabledWeatherMetrics.has("temperature_daily");
  const pressOn   = enabledWeatherMetrics.has("pressure_daily");
  const humidOn   = enabledWeatherMetrics.has("humidity_daily");
    ...
    .upsert(
      {
        user_id: userId,
        date: date,
        temp_c_min:          tempOn    ? cityWeather.temp_c_min          : null,
        temp_c_max:          tempOn    ? cityWeather.temp_c_max          : null,
        temp_c_mean:         tempOn    ? cityWeather.temp_c_mean         : null,
        pressure_hpa_min:    pressOn   ? cityWeather.pressure_hpa_min    : null,
        pressure_hpa_max:    pressOn   ? cityWeather.pressure_hpa_max    : null,
        pressure_hpa_mean:   pressOn   ? cityWeather.pressure_hpa_mean   : null,
        humidity_pct_min:    humidOn   ? cityWeather.humidity_pct_min    : null,
        humidity_pct_max:    humidOn   ? cityWeather.humidity_pct_max    : null,
        humidity_pct_mean:   humidOn   ? cityWeather.humidity_pct_mean   : null,
        wind_speed_mps_mean: windOn    ? cityWeather.wind_speed_mps_mean : null,
        wind_speed_mps_max:  windOn    ? cityWeather.wind_speed_mps_max  : null,
        uv_index_max:        uvOn      ? cityWeather.uv_index_max        : null,
    ...
    for (const job of jobs) {
      ...
      try {
        ...
        const { data: metricRows, error: metricErr } = await supabase
          ...
          .eq("user_id", userId)
          .eq("enabled", true)
          .in("metric", WEATHER_SUB_METRICS);

        if (metricErr) {
          console.warn(`[weather-worker] metric_settings query failed: ${metricErr.message}`);
        }

        const enabledWeatherMetrics = new Set(
          (metricRows ?? []).map((r: any) => String(r.metric))
        );

        // Skip entirely if no weather sub-metrics are enabled
        if (enabledWeatherMetrics.size === 0) {
          await supabase
            .from("sync_jobs_weather")
            .update({ status: "done", updated_at: nowIso })
            .eq("id", jobId);

          results.push({ jobId, status: "skipped_no_metrics" });
          console.log(`[weather-worker] job=${jobId} skipped — no weather sub-metrics enabled`);
          continue;
        }
        ...
        // Pollen + air quality for the whole backfill..forecast window, one query each
        const envStart = addDaysIsoDate(localDate, -13);
        const envEnd = addDaysIsoDate(localDate, FORECAST_DAYS);
        const { data: envPollenRows } = await supabase
          .from("city_pollen_daily")
          .select("*")
          .eq("city_id", nearestCity.cityId)
          .gte("day", envStart)
          .lte("day", envEnd);
        const { data: envAirRows } = await supabase
          .from("city_air_daily")
          .select("*")
          .eq("city_id", nearestCity.cityId)
          .gte("day", envStart)
          .lte("day", envEnd);
        const pollenByDay = new Map((envPollenRows ?? []).map((r: any) => [r.day, r]));
        const airByDay = new Map((envAirRows ?? []).map((r: any) => [r.day, r]));
        ...
        await upsertUserWeatherDay(supabase, userId, cityWeather, nearestCity.cityId, localDate, timezone, nowIso, enabledWeatherMetrics, pollenByDay.get(localDate), airByDay.get(localDate));
          ...
          for (const fRow of forecastRows) {
            try {
              await upsertUserWeatherDay(
                ...
                fRow.day, timezone, nowIso, enabledWeatherMetrics,
                pollenByDay.get(fRow.day), airByDay.get(fRow.day)
              );
              forecastCount++;
            } catch (e: any) {
          ...
          for (const pRow of pastRows) {
            try {
              await upsertUserWeatherDay(
                ...
                pRow.day, timezone, nowIso, enabledWeatherMetrics,
                pollenByDay.get(pRow.day), airByDay.get(pRow.day)
              );
              backfillCount++;
            } catch (e: any) {
