...
async function computeOne(
  ...
  const { data: stressSetting, error: msErr } = await supabase
    ...
    .select("enabled,preferred_source")
    .eq("user_id", userId)
  ...
  try {
    ...
    for (const job of picked) {
      ...
      try {
        ...
        } else if (out.reason === "missing_inputs" && attempts < 10) {
          // HRV/RHR not yet available (sync-worker may still be running).
          // Leave job queued so it retries on the next invocation.
          const nowIso = new Date().toISOString();
          await supabase
            .from("sync_jobs")
            .update({ status: "queued", locked_at: null, last_error: "missing_inputs", updated_at: nowIso })
            .eq("id", job.id);
          results.push({ jobId: job.id, userId, localDate, status: "requeued", reason: "missing_inputs", attempts });
        } else {
